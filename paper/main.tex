...
A Seção organiza-se em duas partes: \textbf{(i)} estimações das regras de Taylor (Seção~\ref{subsec:estimacoes_regra_taylor}); e \textbf{(ii)} implicações para a determinância do equilíbrio no modelo NK (Seção~\ref{subsec:determinancia_bullard_mitra}) \textcolor{magenta}{<-> se inserir * resultados LP, * resultados dynare, * interpretação econômica geral dos resultados (econometria + dynare + teoria + contexto brasil 1999--2024), ADEQUAR AQUI, conforme \ref{subsec:estrategia_estimacao} <->}. 
...
Segundo \cite{woodford2003interest} \textcolor{magenta}{<-> citar a página <->}, quando a política monetária exibe persistência elevada ($\rho \geq 1$), a condição necessária e suficiente para a determinância do equilíbrio deixa de ser uma resposta contemporânea agressiva ($\phi_{\pi} > 1$) e passa a depender da credibilidade da manutenção de taxas de juros elevadas no futuro.
...
\subsubsection{Limitações da Análise Algébrica e Necessidade de Simulação}

Embora a teoria sugira que a super-inércia possa compensar uma resposta contemporânea moderada, a verificação empírica dessa hipótese não é trivial. A desigualdade de Bullard-Mitra apresentada em \ref{subsec:condicoes_determinancia} e \ref{subsubsec:principio_taylor_bullard_mitra} depende implicitamente de multiplicadores finitos de longo prazo. Diante da singularidade matemática gerada pela raiz unitária estimada, a simples substituição dos parâmetros na inequação analítica torna-se inconclusiva.

Para superar essa limitação e responder definitivamente ao problema de pesquisa, faz-se necessário avaliar as \textbf{implicações dinâmicas} dos parâmetros estimados. A subseção a seguir utiliza um modelo DSGE Novo-Keynesiano calibrado para a economia brasileira e simulado via Dynare. O objetivo é submeter o sistema de equações estruturais (NKPC, IS Dinâmica e Regras Estimadas) ao critério de estabilidade de Blanchard-Kahn, verificando computacionalmente se a combinação de parâmetros $(\hat{\rho} \approx 1, \hat{\phi}_{\pi} < 1)$ obtida empiricamente é suficiente para gerar trajetórias convergentes e um equilíbrio determinado, validando o cumprimento do Princípio de Taylor por vias dinâmicas.
