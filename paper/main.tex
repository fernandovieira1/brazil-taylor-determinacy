...
Consideramos três especificações principais da regra de juros, que diferem quanto ao horizonte de previsão e ao grau de inércia da taxa de juros. \footnote{A notação das equações declaradas a seguir foi levemente modificada para manter a consistência com o que foi escrito no artigo, mas sem alteração do sentido original. Em suma, a equação da regra em~\ref{par:regra_taylor_backward} segue a equação (1) de \citeonline{taylor1993}, e as equações das regras descritas em~\ref{par:regra_taylor_forward} e em~\ref{par:taylor_hibrida} seguem respectivamente as equações (9) e (10) de \citeonline{clarida2000monetary}.}
...
\end{equation}
e \(\theta\) reúne os parâmetros de interesse em cada especificação. A validade dos instrumentos e a sobre-identificação são avaliadas pelo teste \(J\) de Hansen, padrão em aplicações de GMM com regras de Taylor \textit{forward-looking}.
