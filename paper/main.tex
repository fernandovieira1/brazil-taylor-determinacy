...
\begin{abstract}
Este artigo investiga a consistência da política monetária brasileira com o Princípio de Taylor e a determinância do equilíbrio macroeconômico sob o Regime de Metas de Inflação (1999--2024). Foram estimadas regras de Taylor \textit{backward-looking}, \textit{forward-looking} e híbridas via GMM, utilizando dados de expectativas de mercado (Relatório Focus) e medidas alternativas de hiato do produto. Os resultados econométricos apontaram para um regime ``super-inercial'', caracterizado por um coeficiente de persistência próximo à unidade ($\rho \approx 0{,}99$) e uma resposta contemporânea à inflação inferior a um ($\phi_\pi < 1$), o que inviabiliza a verificação da estabilidade por critérios algébricos estáticos. Para superar essa limitação, realizou-se uma análise dinâmica utilizando um modelo Novo-Keynesiano simulado no Dynare. As simulações confirmaram que a política monetária brasileira operou em região de determinância entre 1999 e 2024, satisfazendo as condições de Blanchard-Kahn. Os resultados demonstraram que a estabilidade não decorreu de respostas agressivas de curto prazo, mas sim da gestão de expectativas via inércia da taxa de juros, validando a hipótese teórica de Woodford para regimes de alta suavização. Desta forma, o estudo caracteriza a experiência brasileira como uma ``ortodoxia inercial'', onde a credibilidade na persistência do aperto monetário compensou a moderação na resposta inicial aos choques inflacionários.
...
Para este exercício, utilizou-se um modelo DSGE canônico de pequena escala, calibrado com parâmetros estruturais padrão para a economia brasileira ($\beta=0{,}99$, $\sigma=1{,}5$, $\kappa=0{,}08$). A regra de política monetária foi alimentada com a média aritmética dos coeficientes obtidos na estimação híbrida (Tabela~\ref{tab:gmm_hibrido_resultados}), resultando no vetor de parâmetros de simulação: $\hat{\rho} = 0{,}99$, $\hat{\phi}_{\pi} = 0{,}80$ e $\hat{\phi}_{y} = -0{,}05$. Essa escolha busca capturar a tendência central do regime de política monetária no período analisado.
