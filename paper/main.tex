...
O conjunto de variáveis selecionado para a estimação reflete a estrutura canônica do modelo NK adaptada à frequência e às particularidades da economia brasileira, aproveitando os dados explanados na Seção~4. A taxa de juros nominal ($i_t$) é representada pela taxa Selic definida pelo COPOM. A variável de inflação ($\pi_t$) corresponde à variação acumulada em 12 meses do IPCA, servindo como proxy observável para o alvo do regime de metas. Para capturar o estado cíclico da economia ($y_t$), utiliza-se o Índice de Volume do PIB (base 1999=100) como medida de produto real. O hiato do produto é estimado por meio de diferentes metodologias de filtragem estatística, incluindo o filtro Hodrick-Prescott (HP), o filtro de Hamilton e tendências determinísticas (linear e quadrática), conforme ilustrado na Seção~4, permitindo mitigar a incerteza inerente a essa variável não observável. Por fim, as expectativas inflacionárias ($E_t \pi_{t+1}$), cruciais para as especificações \textit{forward-looking}, são representadas pela mediana das projeções de mercado do Relatório Focus para o IPCA do ano corrente, capturando a formação de expectativas em tempo real diante de choques como a Crise Financeira Global de 2008 e a Pandemia de Covid-19.

% -----------------------------------------------------------
\subsubsection{Tratamento das Séries}
% -----------------------------------------------------------

Todas as variáveis são analisadas em frequência trimestral, compatibilizando a periodicidade do PIB, utilizado na construção do hiato do produto, com as séries de inflação, taxa de juros e expectativas. Essa escolha assegura coerência temporal entre as variáveis do modelo e é consistente com o horizonte de transmissão da política monetária, no qual os efeitos das decisões do Copom sobre a atividade e os preços tendem a se materializar ao longo de vários trimestres, e não em flutuações mensais de alta frequência.

As séries de atividade econômica foram previamente ajustadas sazonalmente por meio do procedimento X-13ARIMA-SEATS, metodologia amplamente adotada na análise macroeconômica aplicada. O ajuste sazonal permite remover padrões intra-anuais sistemáticos, como efeitos de calendário, que não estão associados ao ciclo econômico nem à condução da política monetária, facilitando a identificação das flutuações cíclicas relevantes para o estudo do hiato do produto.

- CITAR PAPER E FORMALIZAÇÃO MATEMÁTICA DISSO.

Em seguida, avaliou-se a propriedade de estacionariedade das séries de inflação, taxa de juros e hiato do produto por meio dos testes de raiz unitária Augmented Dickey-Fuller (ADF) e KPSS. Esses testes são empregados de forma complementar na literatura de séries temporais macroeconômicas, uma vez que partem de hipóteses nulas opostas. Sua utilização conjunta permite verificar se as transformações adotadas são compatíveis com a modelagem dinâmica implícita no arcabouço Novo-Keynesiano, no qual inflação e hiato são tratados como variáveis estacionárias em torno de seus valores de equilíbrio de longo prazo.

- CITAR PAPERS E FORMALIZAÇÃO MATEMÁTICA DISSO.
- SITUAR MELHOR ISSO EM RELAÇÃO AO NOSSO ARTIGO.

Com já afirmado, o período amostral compreende o intervalo entre o primeiro trimestre de 1999 e o quarto trimestre de 2024, cobrindo integralmente a vigência do Regime de Metas de Inflação no Brasil. Porém, os primeiros trimestres de 1999, contudo, são analisados com cautela, dada a elevada volatilidade associada à transição cambial e ao processo de consolidação institucional do novo regime monetário.

Dada a natureza não observável do produto potencial e a reconhecida sensibilidade das estimativas do hiato do produto ao método empregado, a medida de hiato utilizada como especificação baseline é obtida por meio do filtro Hodrick--Prescott, com parâmetro de suavização $\lambda = 1600$, padrão para dados trimestrais. A robustez dos resultados é avaliada por meio de especificações alternativas, incluindo o filtro de Hamilton, tendências determinísticas lineares e quadráticas, bem como estimativas institucionais produzidas pela IFI e pelo Banco Central do Brasil. Essa estratégia permite reconhecer explicitamente a incerteza de mensuração inerente ao hiato do produto, ao mesmo tempo em que avalia a estabilidade qualitativa dos resultados empíricos frente a diferentes procedimentos de extração do componente cíclico.

- CITAR PAPERS E FORMALIZAÇÃO MATEMÁTICA DISSO.
- LIGAR ESTE PARÁGRAFO À NOTA DE RODAPÉ DA SEÇÃO 4.
...
Para testar as hipóteses de pesquisa, estimam-se três classes de funções de reação: \textit{backward-looking} (resposta a dados passados), \textit{forward-looking} (resposta a expectativas) e especificações híbridas com suavização da taxa de juros. A equação geral a ser estimada, que aninha essas diferentes hipóteses, é dada por:

\begin{equation}
    i_t = (1-\rho)\left[ \alpha + \phi_\pi E_t\{\pi_{t+k}\} + \phi_y E_t\{\tilde{y}_{t+q}\} \right] + \rho i_{t-1} + \varepsilon_t
\end{equation}

\noindent onde $\rho \in [0, 1)$ é o parâmetro de suavização (inércia da política monetária), $\phi_\pi$ e $\phi_y$ medem, respectivamente, a reação de longo prazo da autoridade monetária à inflação e ao hiato do produto, e $\varepsilon_t$ é o choque exógeno de política monetária.

Quando $k=q=0$ e $\rho=0$, recupera-se a regra de \citeonline{taylor1993}. Para $k>0$ e $\rho>0$, tem-se a especificação \textit{forward-looking} com inércia, conforme \citeonline{clarida2000monetary}, que constitui a especificação \textit{baseline} deste estudo dada a natureza prospectiva do regime de metas.

- LINKAR COM O QUE FOI DITO EM SEÇÕES 2 E 3.

A estratégia de estimação varia conforme a especificação. Para as regras \textit{backward-looking}, utiliza-se o método de Mínimos Quadrados Ordinários (MQO) com matriz de covariância robusta de Newey-West (HAC), corrigindo para autocorrelação e heterocedasticidade nos resíduos. Para as regras \textit{forward-looking}, nas quais as variáveis explicativas (expectativas) são endógenas ou mensuradas com erro, emprega-se o Método dos Momentos Generalizados (GMM). O conjunto de instrumentos $Z_t$ inclui defasagens da taxa de juros, inflação, hiato do produto e índice de \textit{commodities} (IC-Br), variáveis ortogonais ao erro de previsão das expectativas racionais ($E[Z_t (\pi_{t+k} - E_t\pi_{t+k})] = 0$). A validade dos instrumentos é avaliada pelo teste $J$ de Hansen para sobreidentificação.
