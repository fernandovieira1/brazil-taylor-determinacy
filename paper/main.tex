...
\end{equation}
onde \(i_t\) é a meta da taxa Selic, \(\pi_t\) é a inflação medida pelo IPCA, \(\tilde y_t\) é o hiato do produto e \(u_t\) é um erro de política com \(E(u_t|\mathcal F_{t-1})=0\). 

A comparação com a forma teórica com suavização (Seção~\ref{subsec:taylor_hibrida}) permite interpretar
\[
\rho = \beta_1,\qquad
\phi_\pi = \frac{\beta_2}{1-\beta_1},\qquad
\phi_y = \frac{\beta_3}{1-\beta_1},
\]
de modo que \(\phi_\pi\) e \(\phi_y\) são coeficientes de reação de longo prazo à inflação e ao hiato. A estimação é realizada por MQO com matriz de covariância robusta de Newey--West, para acomodar heterocedasticidade e autocorrelação em \(u_t\).

\paragraph{Regra forward-looking (GMM).}
\label{par:regra_forward_gmm}

Na especificação prospectiva, a autoridade monetária reage às expectativas de inflação e de hiato do produto, em consonância com a estrutura forward-looking da NKPC e da equação IS discutidas nas Seções~\ref{subsec:nkpc} e~\ref{subsec:is_dinamica} e com a regra de Clarida--Galí--Gertler da Seção~\ref{subsec:regra_taylor_forward}. A equação estimada é
\begin{equation}
  i_t
  =
  \gamma_0
...
\end{equation}
onde \(E_t\{\pi_{t+1}\}\) é aproximado pela mediana das expectativas Focus para o IPCA em doze meses e \(E_t\{\tilde y_{t+1}\}\) por medidas de hiato prospectivo ou, em especificações restritas, pelo hiato contemporâneo como proxy de informação disponível.

A interpretação de longo prazo segue a mesma lógica:
\[
\rho = \gamma_1,\qquad
\phi_\pi = \frac{\gamma_2}{1-\gamma_1},\qquad
\phi_y = \frac{\gamma_3}{1-\gamma_1},
\]
de forma que \(\phi_\pi\) e \(\phi_y\) podem ser diretamente inseridos na condição de Bullard--Mitra descrita em \ref{subsec:condicoes_determinancia}.

\paragraph{Regra híbrida com suavização (GMM).}
\label{par:regra_hibrida_suavizacao_gmm}
