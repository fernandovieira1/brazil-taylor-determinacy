...
As subseções 5.4.1 a 5.4.3, logo a seguir, descrevem as especificações econométricas correspondentes às diferentes regras de Taylor discutidas na Seção 3.3, enquanto a subseção 5.4.4 explicita de forma unificada a estratégia de estimação e identificação associada a cada uma delas.
...
Em contraste, na especificação \textit{forward-looking}, a autoridade monetária reage explicitamente a expectativas de inflação e de hiato do produto, variáveis endógenas que dependem do mesmo conjunto de informação utilizado na formulação da política e que, em geral, estão correlacionadas com o choque monetário contemporâneo. Nessa situação, a condição de exogeneidade forte requerida pelo MQO é violada, tornando necessária a utilização de estimadores baseados em condições de momento. A estimação por GMM permite explorar instrumentos pertencentes ao conjunto de informação disponível antes da decisão de política, assegurando a consistência dos estimadores mesmo na presença de endogeneidade das expectativas. Essa distinção metodológica segue o aplicado em trabalhos da área, em particular nos trabalhos de \citeonline{clarida2000monetary} e em aplicações mais recentes como \citeonline{hubert2019role} e \citeonline{jayawickrema2019monetary}, que adotam MQO em especificações retrospectivas e GMM em regras prospectivas.
...
Para capturar simultaneamente a resposta à inflação corrente e esperada, bem como a inércia da taxa de juros, estima-se uma regra híbrida alinhada à formulação geral da Seção~\ref{subsec:taylor_hibrida}:
\begin{equation}
  i_t
  =
  \delta_0
...
\end{equation}
Neste caso, define-se \(\rho = \delta_1\) e os coeficientes de reação de longo prazo como
\[
\phi_\pi^{b} = \frac{\delta_2}{1-\delta_1},\qquad
\phi_\pi^{f} = \frac{\delta_3}{1-\delta_1},\qquad
\phi_y = \frac{\delta_4}{1-\delta_1},
\]
de modo que \(\phi_\pi = \phi_\pi^{b} + \phi_\pi^{f}\) sintetiza a agressividade total da política monetária em relação à inflação, combinando componentes backward e forward.

% -----------------------------------------------------------
\subsubsection{Estratégia de estimação}
\label{subsubsec:estrategia_estimacao}
% -----------------------------------------------------------
...
\paragraph{MQO para a regra backward.}
\label{par:mqo_regra_backward}
...
\paragraph{GMM para regras forward e híbrida.}
\label{par:gmm_regras_forward_hibrida}
...
Os parâmetros de reação de longo prazo \(\phi_\pi\) e \(\phi_y\) obtidos a partir das equações \eqref{eq:regra_backward_mqo}--\eqref{eq:regra_hibrida_gmm} são utilizados diretamente para testar as implicações centrais do modelo Novo-Keynesiano.

Em primeiro lugar, o Princípio de Taylor é avaliado por meio do teste de Wald unilateral
\begin{equation}
  H_0:\ \phi_\pi \le 1
...
\end{equation}
onde \(\phi_\pi\) mede a resposta de longo prazo da taxa Selic à inflação, conforme definido na Seção~\ref{subsec:principio_taylor}.

Em segundo lugar, os pares \((\hat\phi_\pi,\hat\phi_y)\) são inseridos na condição de Bullard--Mitra para o modelo NK canônico,
\begin{equation}
  \kappa(\phi_\pi -1) + (1-\beta)\phi_y > 0,
  \label{eq:bullard_mitra_empirica_simplificada}
\end{equation}
em que \(\kappa\) e \(\beta\) são calibrados com base em parâmetros usuais da economia brasileira, conforme discutido na Seção~\ref{subsec:condicoes_determinancia}. Essa desigualdade permite classificar cada especificação e subperíodo da amostra como pertencentes ou não à região de determinância local e estabilidade sob aprendizagem do modelo Novo-Keynesiano.

% -----------------------------------------------------------
\subsection{Testes de Determinância e Quebras Estruturais}
% -----------------------------------------------------------
\label{subsec:testes_determinancia_quebras}

- DEIXAR MAIS ESTATÍSTICO. 

A avaliação do Princípio de Taylor é realizada inicialmente por meio de um teste de Wald unilateral para a hipótese nula $H_0: \phi_\pi \leq 1$ contra a alternativa $H_1: \phi_\pi > 1$. A rejeição da nula indica que o Banco Central cumpre a condição necessária de estabilidade. Contudo, conforme discutido na Seção~\ref{sec:arcabouco_teorico}, a determinância no modelo Novo-Keynesiano exige a satisfação conjunta dos parâmetros na condição de Bullard--Mitra. Assim, verificamos empiricamente se os coeficientes estimados pontuais ($\hat{\phi}_\pi, \hat{\phi}_y$) satisfazem a desigualdade:

\begin{equation}
    \kappa(\hat{\phi}_\pi - 1) + (1-\beta)\hat{\phi}_y > 0
\end{equation}

\noindent utilizando os parâmetros estruturais $\kappa$ e $\beta$ calibrados para a economia brasileira conforme \citeonline{kanczuk2004choques} e OUTRO AUTOR -- JUSTIFICAR.

Por fim, para capturar a heterogeneidade da política monetária ao longo dos diferentes mandatos presidenciais descritos na Seção~\ref{sec:politica_monetaria_brasil_1999_2024}, aplicamos o teste de quebra estrutural de Bai-Perron (\cite{bai1998estimating}; \cite{bai2003computation}). Esse procedimento permite identificar endogenamente os pontos de mudança nos parâmetros da regra de Taylor, validando estatisticamente a segmentação da amostra em subperíodos (Fraga, Meirelles, Tombini, Goldfajn, Campos Neto). A comparação dos coeficientes entre esses regimes permitirá inferir sobre a evolução da aversão à inflação e o compromisso com a determinância ao longo das últimas duas décadas.

- JUSTIFICAR MELHOR, FIZ SÓ O RASCUNHO PRA ORGANIZAR A IDEIA.
...
% Explicar que vai usar Dynare para simular o modelo NK com os parâmetros estimados, pra ver as IRFs e comparar regimes determinantes e indeterminantes.
