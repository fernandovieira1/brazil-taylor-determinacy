...
O conjunto de variáveis selecionado para a estimação reflete a estrutura canônica do modelo NK adaptada à frequência e às particularidades da economia brasileira, aproveitando os dados explanados na Seção~4. \textbf{(I)} A \textbf{taxa de juros nominal} ($i_t$) é representada pela \textbf{taxa Selic} definida pelo COPOM; \textbf{(II)} A variável de inflação ($\pi_t$) corresponde à variação acumulada em 12 meses do IPCA, servindo como proxy observável para o alvo do regime de metas; \textbf{(III)} Para capturar o \textbf{produto} da economia ($y_t$), utiliza-se o Índice de Volume do \textbf{PIB (base 1999=100)}; \textbf{(IV)} O \textbf{hiato do produto} é estimado por meio de diferentes metodologias de filtragem estatística, incluindo o filtro Hodrick-Prescott (HP), o filtro de Hamilton e tendências determinísticas (linear e quadrática), conforme ilustrado na Seção~4 e na subseção a seguir, permitindo mitigar a incerteza inerente a essa variável não observável; \textbf{(V)} Por fim, as \textbf{expectativas inflacionárias} ($E_t \pi_{t+1}$), cruciais para as especificações \textit{forward-looking}, são representadas pela \textbf{mediana} das projeções de mercado do Relatório \textbf{Focus} para o IPCA do ano corrente, capturando a formação de expectativas em tempo real diante de choques exógenos.
