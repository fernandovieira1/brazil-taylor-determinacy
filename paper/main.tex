...
\begin{abstract}
Este artigo investiga a consistência da política monetária brasileira com o Princípio de Taylor e a determinância do equilíbrio macroeconômico sob o Regime de Metas de Inflação (1999--2024). Estimam-se regras de Taylor \textit{backward}, \textit{forward} e híbridas via GMM, utilizando dados de expectativas de mercado (Relatório Focus) e medidas alternativas de hiato do produto. Os resultados econométricos apontam para um regime ``super-inercial'', caracterizado por um coeficiente de persistência próximo à unidade ($\rho \approx 0{,}99$) e uma resposta contemporânea à inflação inferior a um ($\phi_\pi < 1$), o que inviabiliza a verificação da estabilidade por critérios algébricos estáticos. Para superar essa limitação, realiza-se uma análise dinâmica utilizando um modelo Novo-Keynesiano simulado no Dynare. As simulações confirmam que a política monetária brasileira operou em região de determinância, satisfazendo as condições de Blanchard-Kahn. Exercícios contrafactuais demonstram que a estabilidade não decorreu de respostas agressivas de curto prazo, mas sim da gestão de expectativas via inércia da taxa de juros, validando a hipótese teórica de Woodford para regimes de alta suavização. O estudo caracteriza a experiência brasileira como uma ``ortodoxia inercial'', onde a credibilidade na persistência do aperto monetário compensou a moderação na resposta inicial aos choques inflacionários.
