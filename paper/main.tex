...
\label{par:regra_taylor_suavizacao}
...
\[
i_t = \theta i_{t-1} + (1-\theta)\left(\rho + \phi_\pi E_t[\pi_{t+h}] + \phi_y E_t[\tilde{y}_{t+h}]\right) + \varepsilon_t
\]
...
i_t = \theta i_{t-1} + (1-\theta)\left(\rho + \phi_\pi E_t[\pi_{t+h}] + \phi_y E_t[\tilde{y}_{t+h}]\right) + \varepsilon_t
\]
Definindo a taxa real de juros alvo implícita na regra como \(r_t^* \equiv i_t^* - E_t\pi_{t+h}\), obtém-se:
\[
r_t^* = \rho + (\phi_\pi - 1)E_t\pi_{t+h} + \phi_y E_t\tilde{y}_{t+h}
\]
